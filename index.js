...
document.querySelector('#contact-form').onsubmit = (e) => {
	...
	emailjs
		.send('service_iwfvsa5', 'template_5maxbw5', {
		...
		.then(() => {
			...
			let message =
				"primary alert-dismissible mt-4 text-center' role='alert'><i class='bi bi-check-circle-fill me-2'></i>We recieved your message and we are working on it!"
		...
		.catch((err) => {
			...
			let wrapper = document.createElement('div')
			let message =
				"warning alert-dismissible mt-4 text-center' role='alert'><i class='bi bi-exclamation-triangle-fill me-2'></i>We are facing an issue right now. Please try again later. Thank you."
			wrapper.innerHTML = `<div class='alert alert-${message}<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>`
			alertPlaceholder.append(wrapper)
			submitBtn.innerHTML = 'Send'
		})
